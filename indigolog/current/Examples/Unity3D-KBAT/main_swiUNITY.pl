%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% FILE: Unity3D/main_swi.pl
%
%  AUTHOR    : Aaron Woodrow
%  email     : cse02017@cse.yorku.ca
...
:- consult(unity3d).                          % Application code in IndiGolog
...
load_device(Env, Command, Address) :- 
       member((Env,Type), [(unity3d_silent, swi)]),
        (var(Address) -> 
             Host=null, Port=null ; 
             Address = [Host, Port]
        ),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
how_to_execute(Action, unity3d_silent, Action).
...
%translateSensing(_, SensorValue, SensorValue).	% GLEN HAS THIS UNCOMMENTED
...
full_test :-
	...
	once(retract(unity_config(_,_,_,_,_))),
	once(retract(gridsize(_))),
	assert(gridsize(Size)),
		% Set up Size, PPits and NoGolds (only)
	assert(unity_config(test,Size,PPits,NoGolds,scenario)),
	test(301,1),
	fail.
full_test :- 
	writeln('=================================================='),
...
test(Max,N) :-
	...
	tell('/dev/null'),
	retract(unity_config(_,Size,PPits,NoGolds,_)),
		% Assert the type of execution it is going to be done next!
%	assert(unity_config(evalnocut(N),Size,PPits,NoGolds,testnewholds(N))),
	assert(unity_config(nmar05test(N),Size,PPits,NoGolds,random)),
...
:- set_option(wait_step,0).	% TRY CHANGING TO 1.
%:- set_option(debug_level,warn_off).



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EOF: Unity3D/main_swi.pl
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
