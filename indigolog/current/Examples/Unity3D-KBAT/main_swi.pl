...
:- consult(unity3dFinal).           % Application code in IndiGolog
%:- consult(pow). %% SWITCH BACK TO unity3dMidterm or unity3dFinal ONCE POW TEST IS COMPLETE
...
load_device(Env, Command, Address) :- 
       ...
       member((Env,Type), [(unity3d_silent, swi)]),  % use virtual_wumpus to get the xterm console
        (var(Address) -> 
             Host=null, Port=null ; 
             Address = [Host, Port]
        ),
