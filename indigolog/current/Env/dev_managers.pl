...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% UNITY3D_WUMPUS DEVICE 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%unity3d_location('127.0.0.1', 9002).

% With terminal
device_manager(virtual_unity3d_wumpus, swi, Command, [Host, Port]):- 
        main_dir(Dir),
        unity3d_location(IPU, PORTU),
	   unity3d_config(TIDRun),
        concat_atom([Dir,'Env/env_unity3d_wumpus.pl'], File),
        concat_atom([' debug=1', ' ipunity3d=', IPU, ' portunity3d=', PORTU], Options),
		build_call(swi,Host,Port,File,Options,xterm,Command).
...
% With terminal
device_manager(virtual_unity3d, swi, Command, [Host, Port]):- 
        main_dir(Dir),
        unity3d_location(IPU, PORTU),
	   unity3d_config(TIDRun),
	   term_to_atom(TIDRun, IDRun),
        concat_atom([Dir,'Env/env_unity3d.pl'], File),
        concat_atom([' debug=1', ' ipunity3d=', IPU, ' portunity3d=', PORTU, ' idrun=\'', IDRun, '\''], Options),
		build_call(swi,Host,Port,File,Options,xterm,Command).

% Without terminal
device_manager(unity3d_silent, swi, Command, [Host, Port]):- 
        main_dir(Dir),
        unity3d_location(IPW, PORTW),
        wumpus_config(TIDRun,Size,PPits,NoGolds,TIDScenario),
        term_to_atom(TIDRun, IDRun),
        term_to_atom(TIDScenario, IDScenario),
        concat_atom([Dir,'Env/env_unity3dNEW.pl'], File), % if doesn't work then change to env_wumpus.pl and debug.
        concat_atom([' debug=1',
                     ' ipwumpus=', IPW, ' portwumpus=', PORTW, 
                     ' ppits=', PPits, ' nogolds=', NoGolds, ' size=', Size, 
                     ' idrun=\'', IDRun, '\' idscenario=\'', IDScenario,'\''
