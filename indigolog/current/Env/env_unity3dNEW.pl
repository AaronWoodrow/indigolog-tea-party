...
initializeInterfaces(L) :- 
	...
	report_message(system(1), 'Initializing WUMPUS APPLET interface'),
     %   initializeWumpusWorldApplet(IP, Port),
	   confirmUnityConnection(IP, Port),
...
% Unity3D Interface
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
confirmUnityConnection(Host, Port) :-
...
%       send_command_to_wumpus(whosthere, _),
% Used to block and wait for a active connection. 
        read_response_from_wumpus(Data),
		report_message(system(3),
			['Received response from Entity:', Data]). 
...
handle_stream(comm_wumpus) :- 
        ...
        (A=end_of_file ->
        	% Close socket communication with applet (but device manager keeps running with no GUI)
             closeWumpusAppletCom   
        ;
             report_exog_event(A, ['Exogenous action *',A,'* received from Unity3D'])
	).
...
execute2(set_curr(A), _, _, ok) :-
	any_to_string(A,S),
	report_message(action, ['Executing action: *',Action,'*']),
      send_command_to_wumpus(set_curr(S), _).

execute2(set_choice(N,A), _, _, ok) :-
	any_to_string(A,S),
	report_message(action, ['Executing action: *',Action,'*']),
      send_command_to_wumpus(set_choice(N,S), _).


execute2(Action, _, _, ok) :- 
        report_message(action, ['Executing action: *',Action,'*']),
        send_command_to_wumpus(Action, _). %% write(Action)
		%read_response_from_wumpus(Data),
		%report_exog_event(Data, ['Exogenous action *',Data,'* received from Unity3D']).
